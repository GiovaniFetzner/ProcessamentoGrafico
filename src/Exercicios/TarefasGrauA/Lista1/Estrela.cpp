...
const int STAR_POINTS = 5; // Número de pontas da estrela
...
int setupGeometry()
{
    ...
    float r_outer = 0.5f;          // raio externo
    float r_inner = 0.22f;         // raio interno
    const int n = STAR_POINTS * 2; // 10 vértices (alternando externo/interno)
    float vertices[(n + 2) * 3];
    // Primeiro vértice é o centro (opcional para TRIANGLE_FAN, mas não usado aqui)
    // Desenharemos com GL_TRIANGLE_FAN para preencher a estrela
    vertices[0] = cx;
    vertices[1] = cy;
    vertices[2] = 0.0f;
    for (int i = 0; i <= n; ++i)
    {
        ...
        float x = cx + r * cosf(angle - 3.1415926f / 2.0f); // gira para ponta para cima
        float y = cy + r * sinf(angle - 3.1415926f / 2.0f);
        vertices[(i + 1) * 3 + 0] = x;
        vertices[(i + 1) * 3 + 1] = y;
...
int main()
{
    ...
    // Gerando um buffer com a geometria de uma estrela
