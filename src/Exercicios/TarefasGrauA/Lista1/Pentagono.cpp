...
int setupGeometry()
{
    // Aqui setamos as coordenadas x, y e z dos pontos do pentágono e as armazenamos de forma
    // sequencial, já visando mandar para o VBO (Vertex Buffer Objects)
    // O pentágono é desenhado usando a equação paramétrica: x = cx + r*cos(theta), y = cy + r*sin(theta)
    float cx = 0.0f, cy = 0.0f, r = 0.5f;
    float vertices[(PENTAGON_SEGMENTS + 2) * 3];
    // Primeiro vértice é o centro (para TRIANGLE_FAN)
    vertices[0] = cx;
    vertices[1] = cy;
    vertices[2] = 0.0f;
    for (int i = 0; i <= PENTAGON_SEGMENTS; ++i)
    {
        float theta = 2.0f * 3.1415926f * float(i) / float(PENTAGON_SEGMENTS);
...
int main()
{
    ...
    GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Pentagono", nullptr, nullptr);
    if (!window)
    {
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glDrawArrays(GL_TRIANGLE_FAN, 0, PENTAGON_SEGMENTS + 2);
