...
const int PACMAN_SEGMENTS = 50; // Mais segmentos para suavidade
...
int setupGeometry()
{
    // Pac-Man: círculo com "boca" (fatia faltando)
    float cx = 0.0f, cy = 0.0f, r = 0.5f;
    float start_angle = 0.2f * 3.1415926f;             // ângulo inicial (boca)
    float end_angle = 2.0f * 3.1415926f - start_angle; // ângulo final (boca)
    int n = PACMAN_SEGMENTS;
    float vertices[(PACMAN_SEGMENTS + 2) * 3];
    vertices[0] = cx;
    vertices[1] = cy;
    vertices[2] = 0.0f;
    for (int i = 0; i <= n; ++i)
    {
        float theta = start_angle + (end_angle - start_angle) * float(i) / float(n);
