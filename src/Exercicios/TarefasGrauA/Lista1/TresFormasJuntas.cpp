...
int main()
{
    ...
    GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Três formas juntas!", nullptr, nullptr);
    if (!window)
    {
    ...
    // Gerando um buffer com a geometria de um polígono preenchido
    ...
    while (!glfwWindowShouldClose(window))
    {
        // Este trecho de código é totalmente opcional: calcula e mostra a contagem do FPS na barra de título
        {
            ...
            if (title_countdown_s <= 0.0 && elapsed_s > 0.0)
            {
                ...
                char tmp[256];
                sprintf(tmp, "Três formas juntas! \tFPS %.2lf", fps);
        ...
        // Triângulo 1 preenchido - verde claro
        glUniform4f(colorLoc, 0.6f, 1.0f, 0.6f, 1.0f);
        glDrawArrays(GL_TRIANGLES, 0, 3);

        // Triângulo 2 preenchido - cinza claro
        glUniform4f(colorLoc, 0.8f, 0.8f, 0.8f, 1.0f);
        glDrawArrays(GL_TRIANGLES, 3, 3);

        // Contorno dos triângulos com pontos (branco)
        glPointSize(12);
...
int setupGeometry()
{
    ...
    // Dois triângulos preenchidos
    // Triângulo 1: verde claro
    // Triângulo 2: cinza claro
    // Dois triângulos e um círculo de contorno
    const int circle_points = 40;
    GLfloat vertices[3 * 2 * 3 + circle_points * 3];
    ...
    vertices[17] = 0.0f;
    // Círculo (direita)
    float cx = 0.5f, cy = 0.0f, r = 0.25f;
    for (int i = 0; i < circle_points; ++i)
    {
        float theta = 2.0f * 3.1415926f * float(i) / float(circle_points);
        float x = cx + r * cosf(theta);
        float y = cy + r * sinf(theta);
        vertices[18 + i * 3 + 0] = x + 0.5f; // desloca para direita
        vertices[18 + i * 3 + 1] = y;
        vertices[18 + i * 3 + 2] = 0.0f;
    }
